...
type GameModeTwoProps = {
    formValues: Omit<GameSelectionState, 'selectedGameMode' | 'setStartGame'> & { setStartGame: (arg: boolean) => void };
}

export const GameMode2 = ({formValues}: GameModeTwoProps): JSX.Element => {
    const [tileDuration, setTileDuration] = useState<number>(formValues.duration);
    ...
    const updateButtons = (buttonIndex: number) => {
        if (formValues.selectedCharacters[currentCharIndex].equivalents.includes(buttonGroupValues[buttonIndex])) {
            if ((currentCharIndex + 1) <= formValues.selectedCharacters.length) {
                ...
                if ((currentCharIndex + 1) == formValues.selectedCharacters.length) {
                    ...
                    clearTimeout(timer);
                    setTileDuration(formValues.duration);
                    setProgress(((currentCharIndex + 1) / formValues.selectedCharacters.length) * 100);
                }
            }
        } else {
    ...
    const setValuesForButtonGroup = (): void => {
        const values = [];
        let randomCharIndex = getRandomNumberInRange(0, charDB[formValues.characters].length - 1);
        let randomEquivalentIndex = getRandomNumberInRange(0, charDB[formValues.characters][randomCharIndex].equivalents.length - 1);
        values.push(charDB[formValues.characters][randomCharIndex].equivalents[randomEquivalentIndex])
        randomCharIndex = getRandomNumberInRange(0, charDB[formValues.characters].length - 1);
        randomEquivalentIndex = getRandomNumberInRange(0, charDB[formValues.characters][randomCharIndex].equivalents.length - 1);
        values.push(charDB[formValues.characters][randomCharIndex].equivalents[randomEquivalentIndex]);
        randomCharIndex = getRandomNumberInRange(0, charDB[formValues.characters].length - 1);
        randomEquivalentIndex = getRandomNumberInRange(0, charDB[formValues.characters][randomCharIndex].equivalents.length - 1);
        values.push(charDB[formValues.characters][randomCharIndex].equivalents[randomEquivalentIndex]);
        let notFound: boolean = true;
        for (let i = 0; i < formValues.selectedCharacters[currentCharIndex].equivalents.length; i++) {
            if (values.includes(formValues.selectedCharacters[currentCharIndex].equivalents[i])) {
                notFound = false;
            }
        }
        if (notFound) {
            values[getRandomNumberInRange(0, values.length - 1)] = formValues.selectedCharacters[currentCharIndex].equivalents[randomEquivalentIndex];
        }
        ...
        <View>
            ...
            {!timeoutGame && !gameCompleted && !throwOutIncorrect && (
                <>
                    ...
                    >
                        <Text style={{fontSize: 150}}>{formValues.selectedCharacters[currentCharIndex].letter}</Text>
                    </View>
                    <View style={{margin: 5}}>
